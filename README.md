...
4. 把你的学习要求告诉 agent，让它写入 `00-入口/整体学习规划.md` 和 `00-入口/当前学习上下文.md`。
...
1. 第一次使用：把学习目标、每周时间、基础水平和目标期限告诉 Claudian，让它生成整体规划并写入文件。
2. 周初：告诉 Claudian 本周可用时间，让它根据整体规划生成本周 `周计划.md`。
3. 每天开始：在 Claudian 新对话里输入“启动 Java 学习 agent”，先看它汇报当前进度和今天建议。
4. 学习中：让 Claudian 把当天笔记、练习题和代码产出写进对应日期目录，不只停留在聊天记录里。
5. 学习后：告诉 Claudian 今天实际完成了什么，让它更新当天 `学习进度.md`、明天第一步和复习点。
